...
| 介绍一下InnoDB的数据页，和B+树的关系是什么                   |                                                              | [Editorial](./MySQL/介绍一下InnoDB的数据页，和B+树的关系是什么.md) |
...
| 4C8G 16台和 8C16G 8台，不考虑成本的情况怎么选？              |                                                              | [Editorial](./场景题/4C8G16台和 8C16G8台不考虑成本的情况怎么选.md) |
...
| 为什么不直接用原生的 `BlockingQueue` 做消息队列？            |                                                              | [Editorial](./场景题/为什么不直接用原生的 BlockingQueue做消息队列.md) |
| SpringEvent和 MQ的区别及适用场景                             |                                                              | [Editorial](./场景题/SpringEvent和 MQ的区别及适用场景.md)    |
...
| 进入电梯里断网后又恢复刚开始为什么网络慢？                   |                                                              | [Editorial](./场景题/进入电梯里断网后又恢复刚开始为什么网络慢.md) |
...
| 有一张上百万条数据的单表，从前端页面、Java后台、数据库三个层面做查询优化 |                                                              | [Editorial](./场景题/有一张上百万条数据的单表，从前端页面、Java后台、数据库三个层面做查询优化.md) |
| 假设还有很多内存，有什么情况还会频繁fullgc                   |                                                              | [Editorial](./场景题/假设还有很多内存，有什么情况还会频繁fullgc.md) |
...
| 有一个银行系统，对实时性要求比较高，你会怎么选择垃圾回收器   |                                                              | [Editorial](./场景题/有一个银行系统，对实时性要求比较高，你会怎么选择垃圾回收器.md) |
| 百万级会员的用户平台，如何实现快到期的会员的消息提醒         |                                                              | [Editorial](./场景题/百万级会员的用户平台，如何实现快到期的会员的消息提醒.md) |
| A线程获取Redis分布式锁，但那一刻做了主从的切换，B线程能获取到锁吗？ |                                                              | [Editorial](./场景题/A线程获取Redis分布式锁，但那一刻做了主从的切换，B线程能获取到锁吗.md) |
| 如果设计一个缓存，需要考虑哪些方面                           |                                                              | [Editorial](./场景题/如果设计一个缓存，需要考虑哪些方面.md)  |
| 扫表任务，如何写SQL可以避免出现跳页的情况                    |                                                              | [Editorial](./场景题/ 扫表任务，如何写SQL可以避免出现跳页的情况.md) |
...
| Problems                                              | Hints                                                        | Solution                                                     |
| ----------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| xxl-job如何保证一任务只会触发一次？                   |                                                              | [Editorial](./定时任务/xxl-job如何保证一任务只会触发一次.md) |
| xxl-job 支持分片任务吗？实现原理是什么？              |                                                              | [Editoiral](./定时任务/xxl-job支持分片任务的原理.md)         |
...
| 什么是时间轮？                                        |                                                              | [Ediotiral](./定时任务/什么是时间轮.md)                      |
