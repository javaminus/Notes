...
| 只能有一个主键吗？                                           | 一张表只能有一个主键，但是这个主键可以由多个字段组成。（解释第二范式消除部分依赖） |                                                              |
...
| ThreadLocal、Thread、ThreadLocalMap之间的联系                | ThreadLocal就是**保险柜的**钥匙，Thread就是房间，ThreadLocalMap就是保险柜（每个Thread里面都有一个ThreadLocalMap）；                                                             **get()操作：**首先我们会根据**当前的线程**走到对应的房间，然后用ThreadLocal这个钥匙打开ThreadLocalMap保险柜取出我们想要的。                                                              **set()操作：**首先我们会根据**当前的线程**走到对应的房间，然后用ThreadLocal这个钥匙打开ThreadLocalMap保险柜，将我们需要存放的数据放进去。  **每个房间（线程）都有自己的保险柜（ThreadLocalMap），保险柜里可以用不同钥匙（ThreadLocal对象）开不同的格子，每个格子存的是本线程的数据。 **  **钥匙（ThreadLocal）在不同房间能开出不同的数据，互不干扰。**                                                **保险柜的设计让每个线程可以存很多种类型的数据（用不同的钥匙），而且可以自动管理和清理失效的数据（用弱引用防止内存泄漏）。** ThreadLocalMap是线程私有的，如果要添加ThreadLocal到某个线程，只有这个线程的ThreadLocalMap会执行put()操作。 |                                                              |
...
| 谈谈Java的伪共享                                             | **Java的伪共享问题指的是多个线程分别操作各自独立的变量，但这些变量恰好位于同一个CPU缓存行，导致缓存频繁失效和性能下降。虽然线程之间没有真正的数据共享，但由于缓存行的机制，任何一个线程对变量的修改都会让其他线程的缓存失效，从而带来额外的内存同步开销，影响程序的并发性能。在Java中，常见的解决办法包括通过填充变量（padding）或使用JDK 8的@Contended注解，让每个热点变量独占一个缓存行，最大程度减少伪共享带来的性能损失。** | [Editorial](./Java基础/谈谈Java的伪共享.md)                  |
| Java四大引用                                                 | 强、软、弱、虚                                               | [Editorial](./Java基础/四大引用.md)                          |
| 介绍一下websocket                                            | **WebSocket** 是一种网络通信协议，允许浏览器与服务器之间建立持久化的、双向、实时的通信连接。相比传统的 HTTP 请求-响应模式，WebSocket 能够实现客户端和服务器之间的数据实时推送，极大提升了交互效率，特别适用于需要实时数据更新的应用场景，如聊天室、在线游戏、实时股票行情等。 | [Editorial](./Java基础/介绍一下websocket.md)                 |
| Java多线程并发控制面试题集锦                                 |                                                              | [Editorial](./Java基础/Java多线程并发控制面试题集锦.md)      |
...
| 介绍一下虚拟地址                                             | 虚拟地址就是使用**虚拟存储器技术**，程序访问内存时所使用的地址；可以通过页表将虚拟地址映射到物理地址，作用：1、内存隔离与安全性；2、内存扩展性；3、简化编程（开发者不需要关注数据最后实际的物理存储位置） | [Editorial](./操作系统/介绍一下虚拟地址.md)                  |
...
| 如果服务应用部署在 Linux 上，**CPU 打满后**，想查看哪个进程导致的，用什么命令？ | 找进程->找线程->转十六进制->jstack分析线程调用函数栈的情况<br>1、先用 `top`（配合p，按照cpu使用率从高到低排序） 或 `htop` 找到 CPU 占用高的进程对应的 PID。【实时的】<br>2、用 `ps -mp <pid>` 找到该进程下高CPU的线程（TID）。  【-m：显示所有线程。 `-p <pid>`：指定进程ID，只显示该进程及其线程的信息。】<br>3、用 `printf "%x\n" <tid>` 把线程ID转为16进制。 %d是十进制、%x是十六进制、%o是八进制，没有二进制<br>4、用" jstack pid \| grep tid"在线程栈中定位对应线程，查看当前线程的状态（是否出现死锁？分析函数调用情况，观察是否符合预期）分析瓶颈。<br> | [Editorial](./操作系统/如果服务应用部署在Linux上，CPU打满后，想查看哪个进程导致的，用什么命令.md) |
...
| 软链接和硬链接的区别                                         | 都是指向文件地址：软链接（符号链接）是一个指向另一个文件路径的快捷方式，类似于Windows的快捷方式，删除原文件后软链接会失效；硬链接则是为同一个文件创建多个目录入口，所有硬链接平等且指向同一个inode，删除其中任意一个都不会影响文件本身，只有所有硬链接都删除后文件才真正被释放。 | [Editorial](./操作系统/软链接和硬链接的区别.md)              |
