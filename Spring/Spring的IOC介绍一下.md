...
**优点**：
- 依赖强制性高，创建对象时必须传入依赖，保证对象完整性。
- 适合不可变对象，利于线程安全。
- 方便单元测试，可直接传入 mock 对象。

**缺点**：
- 当依赖过多时，构造器参数变多，可读性变差。
- 不适合有可选依赖的情况。

---

### 2. Setter 注入（Setter/属性注入）

**方式说明**：依赖通过 setter 方法注入。
...
**方式说明**：直接通过注解（如 @Autowired）注入到成员变量。

**优点**：
- 编码简洁，无需 setter 或构造器参数。
- 书写方便。

**缺点**：
- 不利于单元测试，难以注入 mock 对象。
- 依赖隐式，阅读和维护性较差。
- 破坏封装性，难以实现不可变对象。
...
**总结**：  
实际开发中，推荐优先使用构造器注入，其次 Setter 注入，不建议使用字段注入，除非特殊场景。构造器注入保证依赖不可或缺，利于测试和维护；Setter 注入适合部分依赖可选的场景；字段注入虽简便但不利于测试和代码质量。

> 在单元测试时，通常需要用 Mock 对象（如 Mockito）来替换实际依赖。如果使用构造器注入或 Setter 注入，可以直接在测试代码中传入 Mock 对象。但字段注入时，依赖是由 Spring 容器通过反射注入到 private 字段，测试代码无法直接赋值，必须用反射或 Spring 的测试工具，操作复杂且不直观。 



> 【扩展】
>
> **Mock对象**，在软件开发（特别是单元测试）中，是“模拟对象”的意思。它是一种用来模拟真实对象行为的替代品，通常用于测试代码时替换掉复杂、难以获得或者有副作用的真实依赖对象。 
>
> ```java
> // 不使用mock
> UserDao userDao = new UserDao(); // 真实DAO
...
> Mockito.when(userDaoMock.findById(1)).thenReturn(new User(1, "Tom"));
> UserService userService = new UserService(userDaoMock);
> User user = userService.getUserById(1);
> // 不会访问真实数据库，返回的是Mock对象
> ```
>
> 
...
**答：**  
- 单例 Bean 的 setter/字段注入循环依赖，Spring 通过三级缓存机制解决。
